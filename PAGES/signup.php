<?php 
    ...
    if(isset($_POST['submit']) && $_POST['g-recaptcha-response'] != "") {
        ...
        if($responseData->success) {
            ...
            $duplicate = mysqli_query($conn, "SELECT * FROM Citizen_Account WHERE Email_Add = '$email' OR national_id_no = '$nat_id' LIMIT 1");
            $admin_duplicate = mysqli_query($conn, "SELECT * FROM Admin_Account WHERE Email_Add = '$email' AND national_id_no = '$nat_id' LIMIT 1");
            ...
            if (mysqli_num_rows($duplicate) > 0 || mysqli_num_rows($admin_duplicate) > 0) {
                ...
                if ($password == $confirmpass) {
                    ...
                    if ($password == "EastRembo_1954") {
                        ...
                        $admin_query = mysqli_query($conn, "INSERT INTO admin_account(Email_Add, Name, Password, National_ID_no, verify_token) VALUES('$email', '$name', '$encrypted_pass', '$nat_id', '$verify_token')");

                        if ($admin_query) {

                            $Home_Address = $houseno." ".$street." ".$brgy;
                            $admin_query1 = "INSERT INTO admin(First_name, Middle_name, Last_name, Birthday, National_ID_no, Sex, Home_Address) VALUES('$fname', '$mname', '$lname', '$bday', '$nat_id', '$value_radio', '$Home_Address')";
                            $admin_query_run1 = mysqli_query($conn,$admin_query1);

                            $sql_result = mysqli_query($conn, "SELECT * FROM admin_account WHERE Email_Add='$email'");

                            if (mysqli_num_rows($sql_result) > 0) {
                                while ($row = mysqli_fetch_assoc($sql_result)) {
                                    $user_id = $row['idAdmin_Account'];
                        ...
                        $name = $fname." ".$lname;
                        $query = "INSERT INTO Citizen_Account(Email_Add, Name, Password, National_ID_no, verify_token) VALUES('$email', '$name', '$encrypted_pass', '$nat_id', '$verify_token')";
                        $query_run = mysqli_query($conn,$query);

                    
                        if ($query_run) {

                            $query1 = "INSERT INTO pwd_senior(VALID_ID_NO, SURNAME, FIRSTNAME, MIDDLENAME, DISABILITY_TYPE, PWD_ID_NO, SENIOR_ID_NO, HOUSE_NO, STREET, BARANGAY, BIRTHDAY, SEX) VALUES('$nat_id', '$lname', '$fname', '$mname', '$disability', '$pwdid', '$seniorid', '$houseno', '$street', '$brgy', '$bday', '$value_radio')";
...
<body onload="document.login-form.reset();">
    <form class="form" action="" method="post" name="signup-form" >
        ...
        <label class="checkbox-container data"> 
            ...
            <a href="dpa.php">View Data Privacy Agreement</a>
        </label>
        </div>
