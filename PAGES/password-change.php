...
if (isset($_POST['submit'])) {
    ...
    if (!empty($token)) {
        ...
        $check_token = mysqli_query($conn, "SELECT verify_token FROM citizen_account WHERE verify_token='$token' LIMIT 1");

        if (mysqli_num_rows($check_token) > 0) {
            
            if ($new_password == $confirmpass) {
                
                $encrypted_new_pass = password_hash($new_password, PASSWORD_DEFAULT);

                $update_password = mysqli_query($conn, "UPDATE citizen_account SET Password='$encrypted_new_pass' WHERE verify_token='$token' LIMIT 1");

                if ($update_password) {
                    
                    $new_token = md5(rand());
                    $update_to_new_token= mysqli_query($conn, "UPDATE citizen_account SET verify_token='$new_token' WHERE verify_token='$token' LIMIT 1");

                    echo
                    "<script> alert('You Successfully changed your password!'); 
                    ...
                    echo
                    "<script> alert('Password Update is Unsuccessful!'); 
                    window.location.href = 'password-change.php?token=$token&email=$email';
                    </script>";
                    exit(0);
                }
            ...
            $admin_check_token = mysqli_query($conn, "SELECT verify_token FROM admin_account WHERE verify_token='$token' LIMIT 1");

            if (mysqli_num_rows($admin_check_token) > 0) {
            
                if ($new_password == $confirmpass) {

                    $encrypted_new_pass = password_hash($new_password, PASSWORD_DEFAULT);
                    
                    $admin_update_password = mysqli_query($conn, "UPDATE admin_account SET Password='$encrypted_new_pass' WHERE verify_token='$token' LIMIT 1");
    
                    if ($admin_update_password) {
                        
                        $new_token = md5(rand());
                        $update_to_new_token= mysqli_query($conn, "UPDATE admin_account SET verify_token='$new_token' WHERE verify_token='$token' LIMIT 1");
    ...
    <form class="form" action="" method="post" name="signup-form">
        ...
        <div class="inputForm">
            ...
            <input type="text" name="email" value="<?php if (isset($_GET['email'])) {
                echo $_GET['email'];
            }?>" id="email" class="input" placeholder="example@gmail.com" required>
        </div>
        ...
        <div class="inputForm">
            ...
            <input type="password" name="new_password" id="password" class="input" placeholder="Enter your Password" required>
        ...
        <button class="button-submit" name="submit" id="submit">Change Password</button>
