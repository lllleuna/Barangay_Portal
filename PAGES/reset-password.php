...
function send_password_reset($get_name, $get_natID, $get_email, $token) {
    ...
    $email_body_template = "
        <h2>RESET YOUR EAST REMBO ACCOUNT PASSWORD</h2>
        <hr/>
        <h3>Name: $get_name</h3>
        <h3>National ID No.: $get_natID</h3>
        <br/>
        <a href='http://localhost/EAST_REMBO/PAGES/password-change.php?token=$token&email=$get_email'>Click here to reset your password</a>
        <br/>
        <h3>If you didn't request a reset, don't worry. You can safely ignore this email.</h3>
...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($check_email) > 0) {
        ...
        if ($update_token) {
            ...
            echo
            "<script> alert('We sent a password reset link to " .$get_email. "'); 
...
<body>
    <form class="form" action="" method="post" name="login-form" >
        ...
        <button class="button-submit" name="submit" id="submit">Send Password Reset Link</button>
